...
days_to_fetch = 3
cache_time_to_live = 1800  # cache exchange data for 30 minutes, configurable in yaml file
...
def fetch_from_exchange(days, config):
    ...
    end = a.default_timezone.localize(EWSDateTime(today.year, today.month, today.day+2))
    ...
    for item in items:
        ...
        datefmt='%B %d, %Y %H:%M'
        result = {'start': item.start.astimezone(tz).strftime(datefmt),
...
if __name__ == '__main__':
    filename = 'ExchangeMeetings/meetings.json'
    ...
    if fileage > cache_time_to_live:
        try:
            meetings = main(days_to_fetch)
            with open(filename, 'w') as text_file:
                json.dump({'meetings': meetings}, text_file, indent=4)
        except:
            if fileage > max_cache_age:
                print(json.dumps({"error": f"unable to fetch update from Exchange in {int(fileage/3600)} hours."}))

    if os.path.isfile(filename):
        with open(filename, 'r') as text_file:
            print(text_file.read())
    else:
        print(json.dumps({"error": f"data file {filename} not found"}))
